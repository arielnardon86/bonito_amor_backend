...
class FacturacionService:
    ...
    def anular_factura(self, factura) -> Tuple[bool, Optional[str]]:
        """
        Anula una factura electrónica emitida.
        
        Args:
            factura: Instancia del modelo Factura a anular
            
        Returns:
            Tuple con (éxito: bool, error: str o None)
        """
        if self.sistema == 'AFIP':
            return self._anular_factura_afip(factura)
        elif self.sistema == 'ARCA':
            return self._anular_factura_arca(factura)
        else:
            return False, "Sistema de facturación no configurado"
    
    def _anular_factura_afip(self, factura) -> Tuple[bool, Optional[str]]:
        """Anula una factura AFIP"""
        if not PYAFIPWS_AVAILABLE:
            return False, "pyafipws no está instalado"
        
        try:
            # Validar que la factura esté emitida
            if factura.estado != 'EMITIDA':
                return False, f"La factura no puede ser anulada. Estado actual: {factura.estado}"
            
            # Validar que tenga los datos necesarios
            if not factura.numero_comprobante or not factura.punto_venta or not factura.cae:
                return False, "La factura no tiene los datos necesarios para anular (falta número de comprobante, punto de venta o CAE)"
            
            # Validar configuración de la tienda
            if not self.tienda.cuit:
                return False, "CUIT no configurado para la tienda"
            ...
            # Convertir tipo de comprobante de letra a código numérico AFIP
            tipo_comprobante_map = {
                'A': 1,   # Factura A
                'B': 6,   # Factura B
                'C': 11,  # Factura C
            }
            tipo_comprobante_codigo = tipo_comprobante_map.get(factura.tipo_comprobante)
            
            if not tipo_comprobante_codigo:
                return False, f"Tipo de comprobante inválido: {factura.tipo_comprobante}"
            
            logger.info(f"Iniciando anulación de factura AFIP")
            logger.info(f"Tipo: {factura.tipo_comprobante} (código: {tipo_comprobante_codigo})")
            logger.info(f"Punto de venta: {factura.punto_venta}")
            logger.info(f"Número: {factura.numero_comprobante}")
            logger.info(f"CAE: {factura.cae}")
            
            # Decodificar certificados (usar la misma lógica que en emitir_factura)
            try:
                cert_b64 = self.tienda.certificado_afip.strip().replace('\n', '').replace('\r', '').replace(' ', '')
                key_b64 = self.tienda.clave_privada_afip.strip().replace('\n', '').replace('\r', '').replace(' ', '')
                ...
                error_msg = f"Error al decodificar certificados: {str(e)}"
                logger.error(f"❌ {error_msg}")
                return False, error_msg
            
            # Autenticar con AFIP (reutilizar la lógica de emitir_factura)
            modo = 'testing' if self.tienda.modo_test_afip else 'production'
            wsaa_url = "https://wsaahomo.afip.gov.ar/ws/services/LoginCms" if modo == 'testing' \
                      else "https://wsaa.afip.gov.ar/ws/services/LoginCms"
            ...
            try:
                ...
                with tf.NamedTemporaryFile(mode='wb', delete=False, suffix='.crt') as cert_file:
                    cert_file.write(cert_data)
                    cert_path = cert_file.name
                
                with tf.NamedTemporaryFile(mode='wb', delete=False, suffix='.key') as key_file:
                    key_file.write(key_data)
                    key_path = key_file.name
                ...
                try:
                    ...
                    if not ta:
                        return False, "Error al autenticar con AFIP para anular factura"
                    
                    # Configurar WSFEv1
                    wsfev1 = WSFEv1()
                    wsfev1.LanzarExcepciones = True
                    wsfev1.Conectar(cache=shared_cache, wsdl=wsaa_url.replace('/LoginCms', '/FEService') if modo == 'testing' else None)
                    wsfev1.SetTicketAcceso(ta)
                    wsfev1.Cuit = self.tienda.cuit.replace('-', '')
                    
                    logger.info(f"✅ Autenticado correctamente. Anulando factura...")
                    
                    # Anular factura
                    # Parámetros: tipo_comprobante, punto_venta, numero_comprobante
                    resultado = wsfev1.AnularFactura(
                        tipo_comprobante_codigo,
                        factura.punto_venta,
                        factura.numero_comprobante
                    )
                    
                    logger.info(f"Resultado de anulación: {resultado}")
                    logger.info(f"Observaciones: {wsfev1.Observaciones}")
                    logger.info(f"Errores: {wsfev1.Errores}")
                    
                    if wsfev1.Errores:
                        error_msg = f"AFIP rechazó la anulación: {wsfev1.Errores}"
                        logger.error(f"❌ {error_msg}")
                        return False, error_msg
                    
                    if resultado:
                        logger.info(f"✅ Factura anulada exitosamente en AFIP")
                        return True, None
                    else:
                        error_msg = f"Error al anular factura en AFIP. Observaciones: {wsfev1.Observaciones}"
                        logger.error(f"❌ {error_msg}")
                        return False, error_msg
                ...
                error_msg = f"Error al anular factura AFIP: {str(e)}"
    ...
    def _anular_factura_arca(self, factura) -> Tuple[bool, Optional[str]]:
        """Anula una factura ARCA (no implementado aún)"""
        return False, "Anulación de facturas ARCA no está implementada aún"
