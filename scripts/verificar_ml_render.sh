...
# Copiar y pegar este código directamente en el shell de Render

cat << 'EOF'
# ============================================
# VERIFICACIÓN DE MIGRACIONES Y CAMPOS ML
# ============================================
# Copia y pega este código en el shell de Render:
...
applied = MigrationRecorder.Migration.objects.filter(app='inventario').values_list('name', flat=True)
...
for c in campos_ml:
    try:
        Tienda._meta.get_field(c)
    except:
        campos_faltantes_modelo.append(c)
